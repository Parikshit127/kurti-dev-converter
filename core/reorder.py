...
# Half-forms (consonant + halant) - used when consonant comes before another consonant
HALF_FORMS = {
    'क': 'D',
    'ख': '[',
    'ग': 'X',
...
# Special Conjuncts (संयुक्ताक्षर) - must be processed BEFORE individual characters
CONJUNCTS = {
    # Ksha group
    'क्ष': '{k',
    'क्ष्': '{',
    ...
    # R-conjuncts (subscript ra) - consonant + ्र
    'क्र': 'Ø',
    'ग्र': 'xz',
    'प्र': 'ç',
    'फ्र': 'Ý',
...
class Tokenizer:
    ...
    def tokenize(cls, text: str) -> List[Tuple[str, str]]:
        ...
        Types: 'hindi', 'english', 'number', 'punctuation', 'whitespace', 'other'
        """
        tokens = []
        current_token = ""
        current_type = None
...
class SyllableParser:
    ...
    def parse_word(cls, word: str) -> List[dict]:
        ...
        Returns list of syllable dicts:
        {
            ...
            'has_reph': True if starts with र्,
            'has_rakar': True if ends with ्र,
        }
        """
        syllables = []
        i = 0
        ...
        while i < n:
            ...
            # Check for Reph (र् at the start)
            if i + 1 < n and word[i] == 'र' and word[i+1] == cls.HALANT:
                # Check if followed by another consonant (not at word end)
            ...
            while i < n:
                ...
                if char == cls.NUKTA:
                    if syllable['consonants']:
                        # Nukta modifies previous consonant
                ...
                if char in cls.CONSONANT:
                    ...
                    if i < n and word[i] == cls.HALANT:
                        ...
                        if i + 1 < n and word[i+1] in cls.CONSONANT:
                            # Check for rakar (्र)
                            if word[i+1] == 'र':
                                if i + 2 >= n or word[i+2] not in cls.MATRA:
                                    # This is rakar (subscript r)
...
# =============================================================================
# SECTION 4: MAPPER (Character to Kruti Dev)
# =============================================================================
...
class Mapper:
    ...
    def map_consonant(cls, consonant: str, is_half: bool = False) -> str:
        ...
        if consonant.endswith('़'):
            ...
            # Fallback: base consonant + nukta marker
...
class SyllableRenderer:
    ...
    def render(cls, syllable: dict) -> str:
        ...
        # The consonants list may have items like 'क्', 'ष' - we need 'क्ष'
        ...
        # Try to match the cluster against known conjuncts
        for conj_unicode, conj_kruti in sorted(CONJUNCTS.items(), key=lambda x: len(x[0]), reverse=True):
        ...
        if not conjunct_found:
            ...
            while remaining:
                ...
                if not matched:
                    ...
                    if remaining:
                        # Get the first consonant (with or without halant)
        ...
        # Add matra (except 'i' which was already added)
...
class MangalToKrutiDevConverter:
    """
    Production-grade Mangal (Unicode) to Kruti Dev converter.
    ...
    def convert(self, text: str) -> str:
        ...
        Returns:
            Text encoded for Kruti Dev font
        """
        if not text:
            return ""
    ...
    def _convert_hindi(self, word: str) -> str:
        ...
        # Pre-process: Replace known conjuncts with markers to preserve them
        # This is done BEFORE syllable parsing to ensure conjuncts are handled correctly
        
        # Sort by length (longest first) to avoid partial matches
        sorted_conjuncts = sorted(CONJUNCTS.items(), key=lambda x: len(x[0]), reverse=True)
        
        # Use a direct replacement approach for known conjuncts
        result = word
        ...
        for conj_unicode, conj_kruti in sorted_conjuncts:
            if conj_unicode in result:
                marker = f'\uE000{marker_idx}\uE001'  # Private use area markers
                markers[marker] = conj_kruti
                result = result.replace(conj_unicode, marker)
                marker_idx += 1
        
        # Now process the remaining text (with markers)
        ...
        for part in parts:
            if part.startswith('\uE000') and part.endswith('\uE001'):
                # This is a marker, replace with the Kruti Dev equivalent
                output_parts.append(markers[part])
            elif part:
                # Process this segment through syllable parser
...
# =============================================================================
# SECTION 7: LEGACY INTERFACE (for compatibility)
# =============================================================================
...
def run_tests():
    """Run test cases to validate conversion."""
    converter = MangalToKrutiDevConverter()
    
    test_cases = [
        # (Input, Description)
        ('भारत', 'Simple word'),
        ('हिंदी', 'Word with i-matra and anusvara'),
        ('हिन्दी', 'Word with explicit half-na'),
        ('नमस्ते', 'Word with conjunct'),
        ('श्री', 'Shri conjunct'),
        ('क्षत्रिय', 'Complex conjuncts'),
        ('विद्यार्थी', 'Word with reph'),
        ('पढ़ाई', 'Word with nukta'),
        ('डॉ.', 'Abbreviation'),
        ('(परीक्षा)', 'Word with brackets'),
        ('कृष्ण', 'Word with ri-matra'),
        ('Hello हिंदी World', 'Mixed language'),
        ('1, 2, 3', 'Numbers and punctuation'),
    ]
    
    print("=" * 60)
    print("MANGAL TO KRUTI DEV CONVERTER - TEST RESULTS")
    print("=" * 60)
